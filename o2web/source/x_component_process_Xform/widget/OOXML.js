...
o2.xApplication.process.Xform.widget.OOXML.WordprocessingML = o2.OOXML.WML = new Class({
        ...
        return this.zip.file("word/document.xml").async("text").then(function (oo_string) {
            ...
            if (this.pics && this.pics.length) {
                return this.zip.file("word/_rels/document.xml.rels").async("text").then(function (oo_relString) {
                    ...
                    return this.zip.file("word/_rels/document.xml.rels", oo_relStr).generateAsync({
                        type: "blob"
                    });
                    //}.bind(this));
                }.bind(this));
    ...
    processWordRel_getPicByBase64: function (pic, idx, oo_doc, oo_relationships) {
        ...
        var fullName = name + "." + ext;
        this.zip.file("word/media/" + fullName, data, {
            "base64": true
        });
        this.processWordRel_createRel(fullName, idx, oo_doc, oo_relationships);
    ...
    createParagraphFromDom: function (dom, oo_body, append) {
        var oo_p = this.createParagraph(oo_body.ownerDocument, {
            "pPrs": this.getPPrs(dom)
        });
    ...
    processTable: function (table, oo_body, append) {
        ...
        grids.each(function (grid) {
            ...
            if (grid) this.setAttrs(oo_gridCol, {
                "w": grid * 20
            });
    ...
    processPic: function (img, oo_p, append) {
        ...
        //     }
        // }
        var oo_run = this.createRun(oo_doc, {
            "rPrs": {
                "noProof": {}
            }
        });
        ...
        if (p === "anchor") {
            ...
            this.setAttrs(oo_simplePos, {
                "x": "0",
                "y": "0"
            }, false);

            var oo_positionH = this.createEl(oo_doc, "positionH", "wp");
            this.setAttrs(oo_positionH, {
                "relativeFrom": "margin"
            }, false);
        ...
        this.setAttrs(oo_graphicFrameLocks, {
            "noChangeAspect": "1"
        }, false);
    ...
    processRunFont: function (node, rPrs, font) {
        ...
        if (msoStyle["mso-font-width"]) rPrs.w = {
            "val": msoStyle["mso-font-width"].toFloat()
        };

        //处理字号
        if (msoStyle["mso-ansi-font-size"]) rPrs.sz = {
            "val": this.parseFontSize(msoStyle["mso-ansi-font-size"]) * 2
        };
        if (msoStyle["mso-hansi-font-size"]) rPrs.sz = {
            "val": this.parseFontSize(msoStyle["mso-hansi-font-size"]) * 2
        };
        if (msoStyle["mso-font-size"]) rPrs.sz = {
            "val": this.parseFontSize(msoStyle["mso-font-size"]) * 2
        };
        if (msoStyle["mso-fareast-font-size"]) rPrs.sz = {
            "val": this.parseFontSize(msoStyle["mso-fareast-font-size"]) * 2
        };
    ...
    processRun: function (span, oo_p, p, text, br) {
        ...
        for (var i = 0; i < keys.length; i++) {
            switch (keys[i]) {
                case "font-size":
                    rPrs.sz = {
                        "val": this.parseFontSize(styles["font-size"]) * 2
                    };
                    break;
                case "color":
                    rPrs.color = {
                        "val": this.getColorHex(styles["color"])
                    };
                    break;
                case "letter-spacing":
                    //实际测试发现letter-spacing * 0.55 转换word比较合适
                    rPrs.spacing = {
                        "val": (styles["letter-spacing"].toFloat() * 20 || 0)
                    };
                    break;
                case "font-weight":
